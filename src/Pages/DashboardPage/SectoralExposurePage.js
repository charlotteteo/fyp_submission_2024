...
const SectoralExposurePage = ({tickers,weights,start_date,end_date,initial_investment}) => {
    ...
    const asyncFetch = async () => {
        try {
            ...
            const timeSeriesResponse = await fetch(`http://localhost:8082/portfolio/sectoral_ts/${tickers_str}/${weights_str}/${start_date}/${end_date}/${initial_investment}`)
            const timeSeriesJson = await timeSeriesResponse.json();
            setData(timeSeriesJson);

            const sectoralDriftTimeSeriesResponse = await fetch(`http://localhost:8082/portfolio/drift_sectoral_ts/${tickers_str}/${weights_str}/${start_date}/${end_date}/${initial_investment}`)
            ...
            const metricsResponse = await fetch(`http://localhost:8082/portfolio/sectoral_metrics/${tickers_str}/${weights_str}/${start_date}/${end_date}/${initial_investment}`)
            const metricsJson = await metricsResponse.json();
            setMetricsData(metricsJson);

            const boxPlotResponse = await fetch(`http://localhost:8082/portfolio/sectoral_box_chart/${tickers_str}/${weights_str}/${start_date}/${end_date}/${initial_investment}`)
            const boxPlotJson = await boxPlotResponse.json();
            setBoxPlotData(boxPlotJson);

            const corrDataResponse = await fetch(`http://localhost:8082/portfolio/sectoral_corr_chart/${tickers_str}/${weights_str}/${start_date}/${end_date}/${initial_investment}`)
...
export default SectoralExposurePage;
